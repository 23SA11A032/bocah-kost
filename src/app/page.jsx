...
function Content() {
    ...
    return (
        <div className="px-3">
            <p className="text-xl font-semibold">Putra</p>
            <div className="grid gap-4">
                {kos.map((v, i) => {
                    if (!parse(v.fotoRumah)?.[0]) return;
                    if (v.jenisKost == "putri") return;
                    return (
                        <div key={i} className="flex flex-col rounded-lg w-48 border border-gray-200 bg-white shadow-md dark:border-gray-700 dark:bg-gray-800 mt-4">
            ...
            <p className="text-xl font-semibold pt-4">Putri</p>
            <div className="grid gap-4">
                {kos.map((v, i) => {
                    if (!parse(v.fotoRumah)?.[0]) return;
